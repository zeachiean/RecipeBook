...
local function OnRecipeClick(self, button)
    -- Right-click: open sources popup listing every source for the recipe
    if button == "RightButton" then
        if self._recipeID and self._profID
            and RecipeBook.RecipeHasAnySources
            and RecipeBook:RecipeHasAnySources(self._profID, self._recipeID) then
            RecipeBook:ShowSourcesPopup(self._profID, self._recipeID)
        end
        return
    end
